...
let filter1vs4 = () => {
    setTimeout(() => {
        ...
        // alert(filter3.value);
        if (filter1.value == "Hammasi" && filter2.value == "Hammasi"
            && filter4.value == "Hammasi"
            && filter3_chek.checked == false || filter3.value == "Hammasi") {
            window.location.href = '/certifcate'
        } else {
            if (!filter3_chek.checked) {
                filter3_demo = "Hammasi";
            }
            window.location.href = `/certifcate/page/1$${filter1.value}$${filter2.value}$${filter3_demo}$${filter4.value}`;
        }
    }, 1000)
}
if (filter1 && filter2 && filter3 && filter4) {
    ...
    filter3_chek.onchange = ()=>{if(filter3_chek.checked && filter3.value.length > 1) filter1vs4()};
    if (filter3_chek.checked) {
