...
router.get('/add', auth, async (req, res) => {
  res.render('public/pages/certifcate/add');
})
...
router.get('/all/delete/:id', auth, async (req, res) => {
  ...
  for (let index = 1; index < ids.length; index++) {
    ...
    let role = await (await db).certificate.getCertificate(element);
    if (!role) {
      continue;
      // return res.render('public/pages/erors/error-404', {
      //   status: 404,
      //   error: element + ' ushbu idga mos hujjat to\'pilmadi!',
      //   path: '/certifcate'
      // });
    }
